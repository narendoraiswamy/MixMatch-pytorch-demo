...
def main():
    ...
    if args.resume:
        ...
        logger.set_names(['Train Loss', 'Train Loss X', 'Train Loss U', 'Train Acc.', 'Valid Loss', 'Valid Acc.', 'Test Loss', 'Test Acc.'])
    ...
    for epoch in range(start_epoch, args.epochs):
       ...
       # bkpt()
        ...
        if epoch%100==0 or epoch==1023:
            plot_acc(eph, train_loss_, val_loss_, test_loss_, train_acc_, val_acc_, test_acc_)
        ...
        # append logger file
        logger.append([train_loss, train_loss_x, train_loss_u, train_acc, val_loss, val_acc, test_loss, test_acc])
...
def plot_acc(eph, train_loss_, val_loss_, test_loss_, train_acc_, val_acc_, test_acc_):

        fig, (ax1, ax2) = plt.subplots(1, 2)
        fig.suptitle('Accuracy and loss subplots')
        ax1.plot(eph, train_acc_, color='blue',label='Train Acc')
        ax1.plot(eph, val_acc_, color='red',label='Val Acc')
        ax1.plot(eph, test_acc_, color='green',label='Test Acc')
        ax1.legend(loc='lower right')
        ax2.plot(eph, train_loss_, color='blue',label='Train loss')
        ax2.plot(eph, val_loss_, color='red',label='Val loss')
        ax2.plot(eph, test_loss_, color='green',label='Test loss')
        ax2.legend('upper right')
        plot_name = './figures/AccLossPlot_'+str(args.n_labeled)+'.png'
        fig_path = './figures/class_name'
